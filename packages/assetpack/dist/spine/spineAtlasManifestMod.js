 ...
 * @returns
 */
export function spineAtlasManifestMod(_options = {}) {
    ...
    return {
        ...
        async finish(asset, options, pipeSystem) {
            const atlasAssets = findAssets((asset) => asset.extension === '.atlas' && asset.transformChildren.length === 0, asset, true);
...
function findAndRemoveManifestAsset(manifest, assetPath) {
    for (let i = 0; i < manifest.bundles.length; i++) {
        const assets = manifest.bundles[i].assets;
        const manifestAsset = assets.find((asset) => asset.src.includes(assetPath));
        if (manifestAsset) {
...
//# sourceMappingURL=spineAtlasManifestMod.js.map